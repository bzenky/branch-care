...
test("confirmed local cleanup uses locked local-only mutation preflight", async (t) => {
  const fixture = makeRepo(); t.after(fixture.cleanup); branch(fixture.dir, "topic"); const client = new GitClient(fixture.dir); const history = new UndoHistory(client); const oid = git(fixture.dir, "rev-parse", "topic"); const paths = await history.paths(); const events: string[] = []; let locked = false; let remoteAccess = 0;
  const acquire = history.acquire.bind(history); history.acquire = async () => { const lock = await acquire(); locked = true; events.push("lock"); const release = lock.release.bind(lock); lock.release = () => { assert.equal(locked, true); events.push("unlock"); locked = false; release(); }; return lock; };
  const reconcile = history.reconcileLocal.bind(history); history.reconcileLocal = async () => { assert.equal(locked, true); events.push("reconcile-local"); return reconcile(); };
  const capacity = history.assertCapacity.bind(history); history.assertCapacity = async (...args) => { assert.equal(locked, true); events.push("capacity"); return capacity(...args); };
  const prepare = history.prepare.bind(history); history.prepare = async (...args) => { assert.equal(locked, true); const pending = await prepare(...args); assert.equal(JSON.parse(readFileSync(resolve(paths.operations, `${pending.id}.json`), "utf8")).state, "pending"); for (const entry of pending.entries) assert.equal(git(fixture.dir, "rev-parse", entry.backupRef), entry.oid); events.push("prepare-durable"); return pending; };
  const code = await runClean({ history, repository: {
    ...
    revalidate: async () => { assert.equal(locked, true); events.push("revalidate"); return { eligible: true }; }, branchOid: async () => oid, deleteBranch: async () => { assert.equal(locked, true); events.push("delete"); },
    resolveRemoteDeletionTarget: async () => { remoteAccess += 1; return undefined; }, remoteHeadOids: async () => { remoteAccess += 1; return new Map(); }
  }, prompts: { select: async () => ["topic"], confirm: async () => true }, output: { out() {}, err() {} }, dryRun: false, interactive: true });
  assert.equal(code, 0); assert.equal(remoteAccess, 0); assert.deepEqual(events, ["lock", "reconcile-local", "capacity", "revalidate", "prepare-durable", "delete", "unlock"]);
});
...
test("confirmed remote cleanup uses locked selected-remote mutation preflight", async (t) => {
  ...
  const stablePaths = await stableHistory.paths(); const events: string[] = []; const inventories: string[] = []; let locked = false; let stableResolutions = 0;
  const acquire = stableHistory.acquire.bind(stableHistory); stableHistory.acquire = async () => { const lock = await acquire(); locked = true; events.push("lock"); const release = lock.release.bind(lock); lock.release = () => { assert.equal(locked, true); events.push("unlock"); locked = false; release(); }; return lock; };
  const reconcile = stableHistory.reconcileRemoteTarget.bind(stableHistory); stableHistory.reconcileRemoteTarget = async (...args) => { assert.equal(locked, true); events.push("reconcile"); return reconcile(...args); };
  const capacity = stableHistory.assertCapacity.bind(stableHistory); stableHistory.assertCapacity = async (...args) => { assert.equal(locked, true); events.push("capacity"); return capacity(...args); };
  const prepare = stableHistory.prepare.bind(stableHistory); stableHistory.prepare = async (...args) => { assert.equal(locked, true); const receipt = await prepare(...args); assert.equal(JSON.parse(readFileSync(resolve(stablePaths.operations, `${receipt.id}.json`), "utf8")).state, "pending"); for (const entry of receipt.entries) assert.equal(git(stable.dir, "rev-parse", entry.backupRef), entry.oid); events.push("prepare"); return receipt; };
  const stableCode = await runRemoteClean({ history: stableHistory, remote: "origin", dryRun: false, interactive: true, repository: {
    resolveRemoteDeletionTarget: async () => { stableResolutions += 1; if (stableResolutions === 2) { assert.equal(locked, true); events.push("endpoint"); } return { name: "origin", urls: [], inventoryRepository: "/origin.git" }; }, analyzeRemoteDeletion: async () => ({ remote: "origin", urls: [], candidates: [{ fullName: "origin/topic", branchName: "topic", oid: stableOid, ageDays: 1 }] }),
    revalidateRemoteDeletion: async (_target, selected) => { assert.equal(locked, true); events.push("revalidate"); return [...selected]; }, deleteRemoteBranches: async () => { assert.equal(locked, true); events.push("push"); return { stdout: "", stderr: "" }; }, remoteHeadOids: async (endpoint) => { assert.equal(locked, true); inventories.push(endpoint); return new Map(); }
  }, prompts: { select: async () => ["origin/topic"], confirm: async () => true, input: async () => "origin" }, output: { out() {}, err() {} } });
  assert.equal(stableCode, 0); assert.deepEqual(events, ["lock", "endpoint", "reconcile", "capacity", "revalidate", "prepare", "push", "unlock"]); assert.deepEqual(inventories, ["/origin.git"]); const operations = await stableHistory.listReadOnly(); assert.equal(operations.find(({ remote }) => remote === "upstream")!.state, "pending");
});
