...
test("full history blocks real local and remote cleanup before side effects", async (t) => {
  ...
  let localWork = 0; let localPrompts = 0; const localLines: string[] = []; const localCode = await runClean({ history, repository: { analyze: async () => { localWork += 1; throw new Error("must not analyze"); }, revalidate: async () => { localWork += 1; return { eligible: true }; }, deleteBranch: async () => { localWork += 1; } }, prompts: { select: async () => { localPrompts += 1; return []; }, confirm: async () => { localPrompts += 1; return false; } }, output: { out: (line) => localLines.push(line), err: (line) => localLines.push(line) }, dryRun: false, interactive: true });
  assert.equal(localCode, 1); assert.equal(localWork, 0); assert.equal(localPrompts, 0); assert.match(localLines.join("\n"), /capacity 10.*undo --list.*undo --discard/s); assert.deepEqual(snapshot(), before);
  let remoteWork = 0; let remotePrompts = 0; const remoteLines: string[] = []; const remoteCode = await runRemoteClean({ history, repository: { resolveRemoteDeletionTarget: async () => { remoteWork += 1; return undefined; }, analyzeRemoteDeletion: async () => { remoteWork += 1; throw new Error("must not analyze"); }, revalidateRemoteDeletion: async () => { remoteWork += 1; return []; }, deleteRemoteBranches: async () => { remoteWork += 1; return { stdout: "", stderr: "" }; } }, prompts: { select: async () => { remotePrompts += 1; return []; }, confirm: async () => { remotePrompts += 1; return false; }, input: async () => { remotePrompts += 1; return ""; } }, output: { out: (line) => remoteLines.push(line), err: (line) => remoteLines.push(line) }, remote: "origin", dryRun: false, interactive: true });
  assert.equal(remoteCode, 1); assert.equal(remoteWork, 0); assert.equal(remotePrompts, 0); assert.match(remoteLines.join("\n"), /capacity 10.*undo --list.*undo --discard/s); assert.deepEqual(snapshot(), before);
});
