...
test("entry point completes output before every platform exit path", async (t) => {
  const source = readFileSync(resolve(process.cwd(), "src/index.ts"), "utf8"); const entry = source.slice(source.indexOf("if (isMainModule())"));
  assert.doesNotMatch(source, /process\.reallyExit\s*\(/); assert.equal((source.match(/process\.exit\s*\(/g) ?? []).length, 1); assert.match(source, /const ENTRY_SIGNALS = \["SIGHUP", "SIGINT", "SIGTERM"\] as const/); assert.match(entry, /removeAddedSignalListeners\(signalListeners\);\s+await finishEntryStdio\(\);\s+if \(process\.platform === "win32"\) process\.exit\(process\.exitCode \?\? 0\)/); assert.equal(entry.indexOf("process.exit(" ) > entry.indexOf("await finishEntryStdio()"), true); assert.match(source, /Promise\.all\(\[finishOutput\(process\.stdout\), finishOutput\(process\.stderr\)\]\)/); assert.match(source, /stream\.once\("finish", complete\)/); assert.match(source, /stream\.end\(\)/); assert.match(source, /process\.stdin\.pause\(\)/); assert.match(source, /stdin\.unref\?\.\(\)/); assert.match(source, /if \(!stdin\.destroyed\) stdin\.destroy\(\)/);
